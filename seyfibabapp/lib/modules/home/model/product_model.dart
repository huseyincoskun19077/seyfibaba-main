...
int parseProductSoldQty(Map<String, dynamic> map) {
  final raw = map['sold_qty'] ?? map['totalSold'] ?? map['total_sold'] ?? 0;
  return int.tryParse(raw.toString().split('.').first) ?? 0;
}

int parseProductSaleUnitQty(Map<String, dynamic> map) {
  final raw = int.tryParse('${map['sale_unit_qty'] ?? 0}') ?? 0;
  return raw > 0 ? raw : 1;
}
