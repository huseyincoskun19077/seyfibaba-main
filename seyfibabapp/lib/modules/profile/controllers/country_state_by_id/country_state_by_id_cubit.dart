...
class CountryStateByIdCubit extends Cubit<CountryStateByIdState> {
  ...
  Future<void> countryListLoaded() async {
    ...
    final result = await _profileRepository.countryList(_loginBloc.userInfo?.accessToken??'');
    result.fold(
          (failure) {
  ...
  Future<void> stateLoadIdCountryId(String id) async {
    ...
    final result = await _profileRepository.statesByCountryId(id, _loginBloc.userInfo?.accessToken??'');
    result.fold(
      (failure) {
        emit(CountryStateByIdStateError(failure.statusCode, failure.message));
      },
  ...
  Future<void> cityLoadIdStateId(String id) async {
    ...
    final result = await _profileRepository.citiesByStateId(id, _loginBloc.userInfo?.accessToken??'');
    result.fold(
      (failure) {
        emit(CountryStateByIdStateError(failure.statusCode, failure.message));
      },
