...
class CartResponseModel extends Equatable {
  ...
  const CartResponseModel({required this.cartProducts});

  CartResponseModel copyWith({List<CartProductModel>? cartProducts}) {
  ...
  factory CartResponseModel.fromMap(Map<String, dynamic> map) {
    return CartResponseModel(
      cartProducts: List<CartProductModel>.from(map['cartProducts']?.map((x) => CartProductModel.fromMap(x))),
    );
  }
