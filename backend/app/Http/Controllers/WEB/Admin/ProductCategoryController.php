...
class ProductCategoryController extends Controller
{
    ...
    {
        $hasMaxInstallment = Schema::hasColumn('categories', 'max_installment');
        $rules = [
        ...
        if($request->image){
            try {
                ...
                Log::error('Category image upload failed on store', [
                    'category_id' => $category->id,
                    'message' => $e->getMessage(),
                ]);
                $notification = array('messege' => 'Kategori görseli yüklenemedi. Sunucu logunu kontrol edin.', 'alert-type' => 'error');
    ...
    public function update(Request $request,$id)
    {
        ...
        if($request->image){
            try {
                ...
                Log::error('Category image upload failed on update', [
                    'category_id' => $category->id,
                    'message' => $e->getMessage(),
                ]);
                $notification = array('messege' => 'Kategori görseli güncellenemedi. Sunucu logunu kontrol edin.', 'alert-type' => 'error');
                return redirect()->back()->withInput()->with($notification);
            }
        }
