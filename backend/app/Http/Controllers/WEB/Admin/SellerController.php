...
class SellerController extends Controller
{
    ...
    public function resendFirstLoginSms(int $id)
    {
        $seller = Vendor::query()
            ->with('user')
            ->findOrFail($id);
        ...
        try {
            $this->registrationService->resendFirstLoginSms($seller);
        } catch (RuntimeException $exception) {
            $notification = ['messege' => $exception->getMessage(), 'alert-type' => 'error'];
    ...
    public function productBySaller($id){
        // Accept vendor id (KYC / ürün özeti) or user id (satıcı detay).
        $seller = Vendor::with('user')->find($id);
        if (! $seller) {
            $user = User::find($id);
            if (! $user) {
                abort(404);
            }
            $seller = Vendor::with('user')->where('user_id', $user->id)->first();
        }
        ...
        return view('admin.product_by_seller', compact('products', 'user', 'seller', 'setting', 'frontend_url'));
    }
