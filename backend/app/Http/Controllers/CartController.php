...
class CartController extends Controller
{
    ...
    {
        $this->middleware('auth:api')->except(['refreshPrices']);
    }
    ...
    /**
     * Sepetteki ürünlerin güncel fiyatlarını döner (Trendyol: canlı fiyat).
     * Misafir ve girişli kullanıcılar için; auth gerekmez.
     */
    public function refreshPrices(Request $request, CartPriceService $cartPriceService)
    {
        $request->validate([
            'items' => 'required|array|min:1',
            'items.*.product_id' => 'required|integer',
            'items.*.qty' => 'nullable|integer|min:1',
            'items.*.variant_item_ids' => 'nullable|array',
            'items.*.variant_item_ids.*' => 'integer',
            'items.*.previous_unit_price' => 'nullable|numeric|min:0',
        ]);

        $payload = collect($request->input('items', []))->map(function ($row) {
            return [
                'product_id' => (int) ($row['product_id'] ?? 0),
                'qty' => (int) ($row['qty'] ?? 1),
                'variant_item_ids' => array_values(array_map('intval', $row['variant_item_ids'] ?? [])),
                'previous_unit_price' => isset($row['previous_unit_price'])
                    ? (float) $row['previous_unit_price']
                    : null,
            ];
        })->all();

        return response()->json($cartPriceService->refreshCartItems($payload));
    }
    ...
    public function addToCart(Request $request){
        ...
        if (!$productStock || $productStock->status != 1 || $productStock->approve_by_admin != 1) {
            $notification = trans('user_validation.Product is not available');
            return response()->json(['message' => $notification ?: 'Bu ürün artık satışta değil.'], 403);
        }
