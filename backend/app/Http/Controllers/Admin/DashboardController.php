...
class DashboardController extends Controller
{
    ...
    public function dashobard(){
        ...
        $todayEarning = round($todayOrders->where('payment_status',1)->sum('amount_real_currency'),2);
        $todayPendingEarning = round($todayOrders->where('payment_status',0)->sum('amount_real_currency'),2);
        $todayProductSale = $todayOrders->where('payment_status',1)->where('order_status',3)->sum('product_qty');
        ...
        $totalEarning = round($totalOrders->where('payment_status',1)->sum('amount_real_currency'),2);
        $totalProductSale = $totalOrders->where('payment_status',1)->where('order_status',3)->sum('product_qty');



        $monthlyOrders = Order::with('user','orderProducts','orderAddress')->orderBy('id','desc')->whereMonth('created_at', now()->month)->get();
        $thisMonthEarning = round($monthlyOrders->where('payment_status',1)->sum('amount_real_currency'),2);
        $thisMonthProductSale = $monthlyOrders->where('payment_status',1)->where('order_status',3)->sum('product_qty');


        $yearlyOrders = Order::with('user','orderProducts','orderAddress')->orderBy('id','desc')->whereYear('created_at', now()->year)->get();
        $thisYearEarning = round($yearlyOrders->where('payment_status',1)->sum('amount_real_currency'),2);
        $thisYearProductSale = $yearlyOrders->where('payment_status',1)->where('order_status',3)->sum('product_qty');
