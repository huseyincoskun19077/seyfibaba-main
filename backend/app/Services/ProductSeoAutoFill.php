...
class ProductSeoAutoFill
{
    ...
    protected function tryAiMeta(string $name, ?string $shortDescription, ?string $longDescription): ?array
    {
        try {
            ...
            $prompt = "Sen Kuaför Tedarik (kuafortedarik.com) SEO yazarısın. Platform: Türkiye’de berber, kuaför, güzellik salonu ve profesyonel kuaför malzemeleri pazaryeri.\n"
                ."Sadece Türkçe yaz. Alakasız ürün/sektör uydurma. Abartılı vaat yok.\n"
                ."Ürün adı: {$name}\n"
                ."Açıklama: {$plain}\n"
                ."seo_title max 60 karakter (ürün ana kelimesi + isteğe bağlı | Kuaför Tedarik).\n"
    ...
    protected function fallbackMeta(string $name, ?string $shortDescription, ?string $longDescription): array
    {
        ...
        if (! str_contains(mb_strtolower($title), 'seyfibaba')) {
            $title = mb_substr($title.' | Kuaför Tedarik', 0, 60);
        }
    ...
    protected function callOpenAiCompatible(Setting $setting, string $prompt): ?string
    {
        ...
        $response = Http::withToken($key)
            ...
            ->post($base.'/chat/completions', [
                'model' => $model,
                'messages' => [
                    ['role' => 'system', 'content' => 'Sen Kuaför Tedarik pazaryeri (berber/kuaför/salon malzemeleri) için Türkçe SEO uzmanısın. Sadece geçerli JSON döndür. Alakasız sektör veya abartılı vaat yazma.'],
