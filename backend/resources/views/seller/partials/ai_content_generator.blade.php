        ...
        <div class="card-body">
            ...
            <div class="d-none" id="ai-success">
                <div class="alert alert-success d-flex justify-content-between align-items-center">
                    <span><i class="fas fa-check-circle mr-1"></i> Icerik basariyla uretildi! Asagidaki alanlara otomatik dolduruldu.</span>
...
(function($) {
    ...
    $(document).on('click', '.ai-action-btn', function(e) {
        ...
        if (!productName && action === 'full') {
            // For full generation, at least short_name is needed
            productName = $('input[name="short_name"]').val();
            if (!productName) {
        ...
        if ((action === 'enhance' || action === 'generate_meta' || action === 'translate') && !productName) {
            toastr.warning('Lutfen once urun adini girin.');
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                ...
                category_name: categoryName !== '{{ __("admin.Select Category") }}' ? categoryName : '',
            ...
            success: function(response) {
                ...
                } else if (response.success && response.generated_content) {
                    // Plain text response
                    toastr.info('AI yaniti alindi ancak JSON olarak ayrıştirilamadi.');
                } else {
            ...
            error: function(xhr) {
                ...
                var msg = 'AI istek hatasi.';
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    msg = xhr.responseJSON.message;
                } else if (xhr.status === 503) {
                    msg = 'AI icerik uretimi su anda kapali. Admin panelden AI ayarlarini aktif edin.';
                } else if (xhr.status === 0) {
                    msg = 'Istek zaman asimina ugradi. Lutfen tekrar deneyin.';
                }
